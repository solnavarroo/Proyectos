...
## Modelo con pie/bicis y camiones pero con deseables agregados
...
def agregar_restricciones(prob, instancia):
    ...
    # Todas las bicis reparten si y solo si entregan 4 pedidos
    for j in range(n):
        vars_entrada = [f"y_{j+1}_{i+1}" for i in range(n) if j != i]
        vars_paradas = [f"p_{j+1}"]
        vars_total = vars_entrada + vars_paradas
        coeficientes = [1.0] * len(vars_entrada) + [-4]

        prob.linear_constraints.add(
            lin_expr=[[vars_total, coeficientes]],
            senses=["G"],
            rhs=[0.0],
            names=[f"mayora4_{j+1}"]
        )
        
    # Los exclusivos son atendidos por camion    
    exclusive = instancia.exclusivos
    for j in range(len(exclusive)):
        vars_entrada = [f"x_{i+1}_{exclusive[j]}" for i in range(n) if exclusive[j] != i+1]
        coeficientes = [1.0] * len(vars_entrada)
        ...
        prob.linear_constraints.add(
            ...
            senses=["E"],
            rhs=[1.0],
            names=[f"exclusive_{exclusive[j]}"]
        )
